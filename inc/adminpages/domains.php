...
function vtm_render_domain_page(){
	...
	<form id="domain-filter" method="get" action='<?php print esc_url($current_url); ?>'>
...
class vtmclass_admin_domain_table extends vtmclass_MultiPage_ListTable {
    ...
    function __construct(){
 	...
 	function add() {
		...
		if ($wpdb->insert_id == 0) {
			echo "<p style='color:red'><b>Error:</b> " . esc_html($_REQUEST['domain_name']) . " could not be inserted (";
			$wpdb->print_error();
			echo ")</p>";
		} else {
			echo "<p style='color:green'>Added " . esc_html($_REQUEST['domain_name']) . "' (ID: " . esc_html($wpdb->insert_id) . ")</p>";
		}
	}
 	...
 	function edit() {
		...
		if ($result) 
			...
			$wpdb->print_error();
			echo "<p style='color:red'>Could not update Domain (" . esc_html($_REQUEST['domain']). ")</p>";
		}
 	...
 	function delete($selectedID) {
		...
		if ($isused) {
			...
			echo "<p style='color:green'>Deleted domain " . esc_html($selectedID) . "</p>";
		}
	}
