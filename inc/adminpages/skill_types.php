...
function vtm_render_skill_types_page(){
	...
	<form id="skill_type-filter" method="get" action='<?php print esc_url($current_url); ?>'>
...
function vtm_render_skill_type_add_form($type, $addaction) {
	...
	<form id="new-<?php print esc_html($type); ?>" method="post" action='<?php print esc_url($current_url); ?>'>
		...
		<tr>
			...
			<td>
				<select name="<?php print esc_html($type); ?>_parentid">
					<option value="0" <?php selected($parentid, 0); ?>>None</option>
					<?php
						foreach (vtm_get_skilltypes() as $sktype) {
							print "<option value='" . esc_html($sktype->ID) . "' ";
							selected($sktype->ID, $parentid);
							echo ">" . esc_html($sktype->NAME) . "</option>";
						}
					?>
				</select>
			</td>
			<td>List Order:</td>
			<td><input type="number" name="<?php print esc_html($type); ?>_ordering" value="<?php print esc_html($order); ?>" size=30 /></td>
		</tr>
		<tr>
			<td>Description:  </td>
...
class vtmclass_admin_skill_type_table extends vtmclass_MultiPage_ListTable {
    ...
    function __construct(){
 	...
 	function add() {
		...
		if ($wpdb->insert_id == 0) {
			echo "<p style='color:red'><b>Error:</b> " . esc_html($_REQUEST['skill_type_name']) . " could not be inserted (";
			$wpdb->print_error();
			echo ")</p>";
		} else {
			echo "<p style='color:green'>Added " . esc_html($_REQUEST['skill_type_name']) . "' (ID: " . esc_html($wpdb->insert_id) . ")</p>";
		}
	}
 	...
 	function edit() {
		...
		if ($result) 
			...
			$wpdb->print_error();
			echo "<p style='color:red'>Could not update Ability Category (" . esc_html($_REQUEST['skill_type']) . ")</p>";
		}
 	...
 	function delete($selectedID) {
		...
		if ($isused) {
			...
			echo "<p style='color:green'>Deleted Ability Category " . esc_html($selectedID) . "</p>";
		}
	}
