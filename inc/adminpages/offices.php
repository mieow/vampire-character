...
function vtm_render_office_page(){
	...
	<form id="office-filter" method="get" action='<?php print esc_url($current_url); ?>'>
...
function vtm_render_office_add_form($type, $addaction) {
	...
	<form id="new-<?php print esc_html($type); ?>" method="post" action='<?php print esc_url($current_url); ?>'>
		...
		<tr>
			...
			<td>Ordering:</td>
			<td><input type="text" name="<?php print esc_html($type); ?>_ordering" value="<?php print esc_html($ordering); ?>" size=5 /></td>
...
class vtmclass_admin_office_table extends vtmclass_MultiPage_ListTable {
    ...
    function __construct(){
 	...
 	function add() {
		...
		if ($wpdb->insert_id == 0) {
			echo "<p style='color:red'><b>Error:</b> " . esc_html($_REQUEST['office_name']) . " could not be inserted (";
			$wpdb->print_error();
			echo ")</p>";
		} else {
			echo "<p style='color:green'>Added " . esc_html($_REQUEST['office_name']) . "' (ID: " . esc_html($wpdb->insert_id) . ")</p>";
		}
	}
 	...
 	function edit() {
		...
		if ($result) 
			...
			$wpdb->print_error();
			echo "<p style='color:red'>Could not update Office (" . esc_html($_REQUEST['office']) . ")</p>";
		}
 	...
 	function delete($selectedID) {
		...
		if ($isused) {
			...
			echo "<p style='color:green'>Deleted office " . esc_html($selectedID) . "</p>";
		}
	}
